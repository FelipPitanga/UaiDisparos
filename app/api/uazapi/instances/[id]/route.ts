...
export async function DELETE(_request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const { data: record, error } = await supabase
      .from("instances")
      .select("id,name,base_url,api_token")
      .eq("id", params.id)
      .single();
    ...
    if (record.base_url && record.api_token) {
      ...
      if (!response.ok && response.status !== 404) {
        ...
        try {
          ...
          // Mantém o texto bruto para diagnóstico.
        }

        return NextResponse.json(
          {
            ok: false,
            error: `A UAZAPI não conseguiu excluir a instância (${response.status}): ${
    ...
    const { error: deleteError } = await supabase.from("instances").delete().eq("id", record.id);
    ...
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : "Erro ao excluir instância." },
      { status: 500 },
    );
  }
}
